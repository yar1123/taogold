...
KISSY.use("taogold/preview,taogold/dialog",function(S, Preview, Dialog){
    ...
    var useTips = '启用该模板，将往您的所有宝贝描述里插入您设定的推荐内容。确定要启用吗？',
        stopTips = '禁用该模板，将从您的所有宝贝描述里删除您设定的推荐内容。确定要禁用吗？';
           
    var alertDialog = new Dialog({type:'alert', title:'提示', width:400}),
        confirmDialog = new Dialog({type:'confirm', title:'操作确认', width:400});
    ...
    confirmDialog.on('confirm',function(){
        S.io.get(
            ...
            function(o){
                var data = eval('['+o+']');
                alertDialog.appendContent(D.create('<div style="padding:10px;">'+ (D.hasClass(currentTrigger,'S_UseTrigger') ? '正在启用' : '正在禁用')+'，整个处理过程大约需要<span class="h">'+data[0].ti+'</span>分钟。<br/>届时我们会为您自动刷新页面。<br/>您也可以点击操作栏的<a href="/top/rechome.html">刷新页面</a>按钮进行手动刷行。</div>'));
                alertDialog.show();
