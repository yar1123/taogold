...
SECRET_KEY = '5pe+co)yh0cv0sn4xa3r!^w$ydr$=th=)bl%rx47o)*$)^gsb#'
...
LOGGING = {
    ...
    'handlers': {
        ...
        'top_log_request_handler' : { 
            'class' : 'logging.handlers.TimedRotatingFileHandler',
            'level' : 'DEBUG',
            'when' : 'H',
            'filename' : os.path.join(os.path.abspath('./logs'), 'top.log.request'),
            'formatter' : 'top_log',
        },
        'mail_admins': {
            'level': 'ERROR',
    ...
    'loggers': {
        'django': {
            'handlers':['top_log_handler'],
            'propagate': True,
            'level':'DEBUG',
        },
        'django.request': {
            'handlers': ['top_log_request_handler' ],
            'level': 'DEBUG',
            'propagate': True,
        },
    }
